functions{
    real reducer( 
            ...
            int[] A , int[] S , int[] FS , int D , 
            int L , int Q ,int R ,
            // parameters to pass
            vector mA , matrix aK ,
            vector aK_sigma , matrix bA ,  vector bFS,
            matrix delta_j , 
...
model{
  ...
  //model
	target += reduce_sum( reducer , A , 1 , 
      // data to pass
			Y_l , Y_q , Y_r , A , S , FS , D , L , Q , R ,
      // parameters to pass
      mA , aK , aK_sigma , bA , bFS , delta_j , a_l , b_l , a_q , b_q , a_r , b_r, c_q );
}//model
 ...
 generated quantities {
   ...
   for ( d in 1:D ) 
      ...
      K[i,d] = mA[d] +                                           //global intercept - minimum value of knowledge
               ...
               bFS[d] * FS[i] +                                  //effect of being firstborn
