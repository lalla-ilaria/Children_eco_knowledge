functions{
    real reducer( 
            ...
            int start , int end , 
            // data
            int[,] Y_l , int[,]Y_q , int[,] Y_r , int[] A , row_vector[] AM , int[] S , int D , int L , int Q ,int R ,
            // parameters to pass
            vector mA , matrix aK , 
   ...
   for ( i in start:end ) {
      ...
      logit_p = log( exp(p) + c_q ) - log1m( c_q );
		  ...
		  lp = lp + bernoulli_logit_lpmf( Y_q[i,] | logit_p );
    }//i
   for ( i in start:end ) {
...
model{
  ...
  //model
	target += reduce_sum( reducer , A , 1 , 
      // data to pass
			Y_l , Y_q , Y_r , A , AM , S , D , L , Q , R ,
      // parameters to pass
      mA , aK , aK_sigma , bA , delta_j , aAM , a_l , b_l , a_q , b_q , a_r , b_r , c_q );
}//model
