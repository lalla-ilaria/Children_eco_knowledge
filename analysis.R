...
dat_list <- list( 
  ...
  adults = rep( df_ID$fam_adults, each = sp),
  HH = rep(df_ID$household, each = sp),
  activity_matrix = matrix_IDactivity,
...
m1.8 <- ulam( 
  alist(
    ...
    logit(p) <- a_bar + a[ID]*sigma_a + s[spp_id]*sigma_s + bA*age + bS*school_years + bY*young_sib + bO*old_sib + bAd*adults + h[HH]*sigma_h +
      ...
      AE[ 1 , spp_id[i] ]*activity_matrix[i,1] +
      AE[ 2 , spp_id[i] ]*activity_matrix[i,2] +
      AE[ 3 , spp_id[i] ]*activity_matrix[i,3] +
      AE[ 4 , spp_id[i] ]*activity_matrix[i,4] +
      AE[ 5 , spp_id[i] ]*activity_matrix[i,5] +
      AE[ 6 , spp_id[i] ]*activity_matrix[i,6] +
      AE[ 7 , spp_id[i] ]*activity_matrix[i,7] +
      AE[ 8 , spp_id[i] ]*activity_matrix[i,8] +
      AE[ 9 , spp_id[i] ]*activity_matrix[i,9] +
      AE[ 10 , spp_id[i] ]*activity_matrix[i,10] ,
    a[ID] ~ dnorm( 0 , 1 ),
    a_bar ~ dnorm( 0 , 1.5 ), 
    s[spp_id] ~ dnorm( 0 , 1 ),
    h[HH] ~ dnorm( 0 , 1 ),
    ...
    bO ~ dnorm ( 0 , 1 ),
    bAd ~ dnorm ( 0 , 1 ),
    ex[sex] ~ dnorm( 0 , sigma_ex ),
    sigma_a ~ dexp( 1 ),
    sigma_s ~ dexp( 1 ),
    sigma_ex ~ dexp( 1 ),
    sigma_h ~ dexp( 1 ),
    matrix[10,N_spp]:AE ~ normal(0,1)
  ) , data=dat_list , chains=1 , cores=4 , log_lik=TRUE , sample=TRUE )
