...
```{r dimensions everything together, cache=TRUE}
#all types of data
D <- c(1:3)
m_da <- list()
...
for (i in 1:length(D)) {
  ...
  m_d_alldata[[i]] <- stan( file = "models/1_dimensions_intercept_only_all_items.stan", data=dat , chains=3, cores=3, init = 0 )
}
#model comparison
...
```{r lets see only age effect, cache=TRUE}
#freelist only
#run the model with 1:3 number of dimensions
